...
my $BASE = $ENV{TEST_URL} || 'http://139.162.204.109:5030';
...
subtest 'GET tests that modify data' => sub {
  plan tests => 5;
...
subtest 'GET tests requiring cookie' => sub {
  plan tests => 1;

  #$mech->get_ok( $BASE . '/author/comments/' . $status . '/page/' . $page      );
  #$mech->get_ok( $BASE . '/author/comments/page/' . $page              );
  #$mech->get_ok( $BASE . '/author/comments'                              );

  #$mech->get_ok( $BASE . '/author/posts'                                 );
  #$mech->get_ok( $BASE . '/author/posts/page/' . $page                 );
  #$mech->get_ok( $BASE . '/author/posts/:status/page/' . $page         );
};
...
subtest 'GET tests' => sub {
  ...
  $mech->get_ok( $BASE . '/admin/comments/page/' . $page                 );
  $mech->get_ok( $BASE . '/admin/comments/' . $status . '/page/' . $page );
  $mech->get_ok( $BASE . '/admin/comments'                               );
  $mech->get_ok( $BASE . '/admin/posts/edit/' . $slug                    );
  $mech->get_ok( $BASE . '/admin/posts/page/' . $page                    );
  ...
  $mech->get_ok( $BASE . '/admin/users/page/' . $page                    );
  $mech->get_ok( $BASE . '/admin/users/' . $status . '/page/' . $page    );
  $mech->get_ok( $BASE . '/admin/users'                                  );
  #$mech->get_ok( $BASE . '/api/categories.:format'                       );
  #$mech->get_ok( $BASE . '/api/tags.:format'                             );
  $mech->get_ok( $BASE . '/avatars/' . $avatar_path                      );
  ...
  $mech->get_ok( $BASE . '/page/' . $page                                );
  $mech->get_ok( $BASE . '/password_recovery'                            );
  $mech->get_ok( $BASE . '/posts/category/' . $slug . '/page/' . $page   );
  $mech->get_ok( $BASE . '/posts/category/' . $slug                      );
  $mech->get_ok( $BASE . '/post/' . $slug                                );
  $mech->get_ok( $BASE . '/posts/tag/' . $slug . '/page/' . $page        );
  $mech->get_ok( $BASE . '/posts/tag/' . $slug                           );
  $mech->get_ok( $BASE . '/posts/user/' . $username . '/page/' . $page   );
  ...
  $mech->get_ok( $BASE . '/search/posts/' . $query . '/' . $page         );
